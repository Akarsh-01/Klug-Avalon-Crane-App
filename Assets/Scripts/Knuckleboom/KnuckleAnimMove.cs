...
public class KnuckleAnimMove : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        button1 = allMovement.GetButton1Value();
        if (knuckleMove > 0 && button1 == buttonValue && allMovement.GetButton2Value() == 0)
        {
            direction = 1f;
            currentFrame += direction * animSpeed * Time.deltaTime * (totalFrames / animator.GetCurrentAnimatorStateInfo(animationLayer).length);
        }
        else if (knuckleMove < 0 && button1 == buttonValue && allMovement.GetButton2Value() == 0)
        {
            direction = -1f;
            currentFrame += direction * animSpeed * Time.deltaTime * (totalFrames / animator.GetCurrentAnimatorStateInfo(animationLayer).length);
        }
